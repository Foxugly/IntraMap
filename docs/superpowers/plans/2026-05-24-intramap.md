...
**Architecture:** Four isolated modules — `scanner` (python-nmap wrapper → `DiscoveredHost`), `inventory` (YAML persistence + intelligent merge), `renderers/*` (`Inventory` → text), and `cli` (orchestration). MAC address is the identity key for hosts; the YAML file is editable by hand and merge preserves user annotations (custom name, location, declared uplink to a switch with optional PoE flag and patch-panel port).
...
### Task 3: Dataclasses (Location, Uplink, DiscoveredHost, Host, Inventory)

All five dataclasses + `to_dict`/`from_dict` for YAML round-trip. MAC is normalized at construction (on both `Host.mac` and `Uplink.switch_mac`).
...
Run: `pytest tests/test_models.py -v`
Expected: 18 passed (10 from Task 2 + 8 added here).

- [ ] **Step 5: Commit**

```bash
git add intramap/models.py tests/test_models.py
git commit -m "feat: data model (Location, Uplink, Host, Inventory, DiscoveredHost)"
```
...
Hosts grouped by `floor` → `room` → `rack`. Hosts with no `floor` go to a "Non localisé" group. A floor with a host that has no `room` gets a "(sans pièce)" room. Offline hosts get a stereotype `<<offline>>` styled in grey. For each host with a declared `uplink.switch_mac` referencing a known inventory host, an edge is drawn to that switch with a label combining `sw:<port>`, `pp:<port>`, and `PoE` (when set). PoE edges are styled with `[#orange,thickness=2]`.
...
git commit -m "feat: PlantUML renderer with floor/room/rack groups and uplink edges"
```
...
Same grouping logic, output as DOT with nested `subgraph cluster_*` blocks. Offline hosts get `style=dashed,color=grey`. Labels escape `"`. Edges drawn for declared uplinks pointing to known hosts; PoE edges get `color="orange", penwidth=2`.
...
Run: `pytest tests/test_renderers.py -v`
Expected: 18 passed (10 from Task 8 + 8 added here).

- [ ] **Step 5: Commit**

```bash
git add intramap/renderers/graphviz.py tests/test_renderers.py
git commit -m "feat: Graphviz DOT renderer with floor/room/rack groups and uplink edges"
```
...
- **Spec coverage:** every spec section maps to tasks — §4 architecture → Task 1 (scaffolding) + module split across all tasks; §5 model (including `Uplink`) → Tasks 2-3; §6 merge (preserving `uplink`) → Task 6; §7.1 scan flow → Task 12; §7.2 render flow (with edges for declared uplinks, PoE styling, unknown-MAC skip) → Tasks 8-9, 11; §7.3 list → Task 10; §7.4 PlantUML example → Task 8 tests; §7.5 declared-only topology → enforced by design (renderer reads `uplink` field only, never queries the network); §8 CLI surface → Tasks 10-12; §9 error frontiers → covered in Tasks 5 (corrupt YAML, atomic), 7 (nmap missing), 10/11/12 (CLI messages); §9.1 atomic write → Task 5; §10 tests → every task is TDD with explicit coverage of uplink merge preservation, PoE edge styling, and unknown-MAC silent skip; §11 deps → Task 1 pyproject.toml.
- **Placeholder scan:** none — every step shows actual code or actual commands.
- **Type consistency:** `DiscoveredHost`, `Host`, `Inventory`, `Location`, `Uplink` use the same field names across all tasks; `merge(inv, discovered, now=now)` signature is consistent in Tasks 6 and 12; `node_ids` mapping and `_edge_label` helper are introduced consistently in both renderers (Tasks 8 and 9).
